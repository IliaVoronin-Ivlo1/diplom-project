...
app = FastAPI(
    title="Price Analysis Service",
    description="Анализ цен и ценообразования",
    lifespan=lifespan
)
...
async def root():
    return {
        "service": "price-analysis-service",
        "description": "Анализ цен и ценообразования",
        "status": "running"
    }
...
@app.post("/analyze")
async def analyze_prices(data: dict):
    return {
        "service": "price-analysis-service",
        "message": "Анализ цен выполнен",
        "data": data
    }
...
if __name__ == "__main__":
    ...
    logger.info(f"Main[__main__] Starting price analysis service on port {port}")
