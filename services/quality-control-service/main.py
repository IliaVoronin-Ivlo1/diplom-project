...
app = FastAPI(
    ...
    description="Контроль качества и проверка",
    lifespan=lifespan
)
...
@app.post("/check")
async def check_quality(data: dict):
    return {
        "service": "quality-control-service",
        "message": "Проверка качества выполнена",
        "data": data
    }
...
if __name__ == "__main__":
    ...
    logger.info(f"Main[__main__] Starting quality control service on port {port}")
